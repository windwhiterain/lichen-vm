...
pub struct Project;
impl ::lichen_core::plugin::Project
 for Project{
type Value<> = Value<>;
...
pub(super) core__equality_error: std::mem::ManuallyDrop<::lichen_core::runtime::diagnostic::EqualityError::<>>,
}
}
...
match self.0{
0=>{write!(f,"core::sum")}1=>{write!(f,"core::index")}2=>{write!(f,"core::find")}_=>unreachable!(),}
}
}
impl ::lichen_core::plugin::principal_traits::Operator
::<Project> for Operator{
fn run<>(& self,solver: &mut ::lichen_core::runtime::solve::Solver<Project>,value: & <Project as ::lichen_core::plugin::Project
>::Value,node: & ::lichen_core::runtime::solve::LocalNodeId,)->Option<<Project as ::lichen_core::plugin::Project
>::Value>{
match self.0{
...
#[derive(Clone,Copy,)]
pub struct Value<>{code:usize,data:self::unions::Value<>}
impl Eq for Value{}
...
fn fmt<>(& self,f: &mut std::fmt::Formatter<'_>,)->std::fmt::Result{
match self.code{
0=>{write!(f,"core::int({:?})",unsafe{&*self.data.core__int})}1=>{write!(f,"core::string({:?})",unsafe{&*self.data.core__string})}2=>{write!(f,"core::array({:?})",unsafe{&*self.data.core__array})}3=>{write!(f,"core::table({:?})",unsafe{&*self.data.core__table})}4=>{write!(f,"core::unit")}_=>unreachable!(),}
}
}
...
fn for_field_pairs<>(& self,other: & Self,mut action: impl FnMut(&::lichen_core::runtime::NodeIdLocal,&::lichen_core::runtime::NodeIdLocal),){
match self.code{
...
match self.code{
0=>{unsafe{&self.data.core__equality_error}.hash(state);
}_=>unreachable!(),}
}
}
...
fn clone<>(& self,)->Self{
match self.code{
0=>{Self{code:self.code,data:self::unions::DiagnosticKind{core__equality_error:unsafe{&self.data.core__equality_error}.clone() } }}_=>unreachable!(),}
}
}
...
fn equality_error(&self)->Option<&::lichen_core::runtime::diagnostic::EqualityError::<>>{if self.code==0{Some(unsafe{&self.data.core__equality_error})}else{None} }
fn from_equality_error(data: ::lichen_core::runtime::diagnostic::EqualityError::<>)->Self{Self{code:0,data:self::unions::DiagnosticKind{core__equality_error:std::mem::ManuallyDrop::new(data)} } }
}
pub struct Ast<'a>(::lichen_core::AstImpl<'a,Project>);
impl ::lichen_core::plugin::Ast for Ast<'_>{
fn value(&self,expr:&::lichen_core::ExprId)->::lichen_core::runtime::NodeIdLocal{self.0.property(expr,0)}
}
impl ::lichen_core::Ast for Ast<'_>{
