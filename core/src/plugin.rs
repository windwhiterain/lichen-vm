...
fn fields<>(& self,)->impl Iterator<Item=&crate::runtime::NodeIdLocal>{std::iter::empty()}
fn for_fields<>(& self,mut action: impl FnMut(&crate::runtime::NodeIdLocal),){for i in self.fields(){{action(i);}}}
fn for_field_pairs<>(& self,other: & Self,mut action: impl FnMut(&crate::runtime::NodeIdLocal,&crate::runtime::NodeIdLocal),){for (i,j) in self.fields().zip(other.fields()){{action(i,j);}}}
}
pub trait Operator<Project: crate::plugin::Project
+>:std::fmt::Debug+Copy+Eq+{
fn run<>(& self,solver: &mut crate::runtime::solve::Solver<Project>,value: & <Project as crate::plugin::Project
>::Value,node: & crate::runtime::solve::LocalNodeId,)->Option<<Project as crate::plugin::Project
>::Value>;
}
