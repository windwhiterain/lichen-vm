...
fn main() {
    ...
    let structures = Tuple::new(ast.module_mut(), [unit, unit]);
    ...
    let members = Tuple::new(ast.module_mut(), [member0, member1]);
    ...
    let composed_structure_value = ast.get_value(&composed_structure);
    let composed_structure_value_target = Value::from_structure(Structure {
        layout: Table::new(ast.module_mut(), [string0, string1]),
        components: Tuple::new(ast.module_mut(), [unit, unit]),
    });
    assert_eq!(
    ...
    let member0_value = ast.get_value(&member0);
    assert_eq!(
    ...
    let member1_value = ast.get_value(&member1);
    assert_eq!(
