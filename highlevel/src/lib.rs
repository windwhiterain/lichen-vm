//! The highlevel layer: a typed builder that constructs lowlevel [`Module`]s
...
//! [`checker::Checker`] checks it and builds the lowlevel [`Module`] in one
//! pass — unify is runtime behaviour, so checking happens while building.
//!
